...
from kio.schema.offset_commit.v9.request import OffsetCommitRequest
from kio.schema.offset_commit.v9.request import OffsetCommitRequestPartition
from kio.schema.offset_commit.v9.request import OffsetCommitRequestTopic
...
def test_offset_commit_request_partition_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_offset_commit_request_partition(
            buffer.getbuffer(),
        )
...
def test_offset_commit_request_topic_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_offset_commit_request_topic(
            buffer.getbuffer(),
        )
...
def test_offset_commit_request_roundtrip(instance: OffsetCommitRequest) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_offset_commit_request(
            buffer.getbuffer(),
        )
