...
from kio.schema.create_partitions.v0.response import CreatePartitionsResponse
from kio.schema.create_partitions.v0.response import CreatePartitionsTopicResult
...
def test_create_partitions_topic_result_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_create_partitions_topic_result(
            buffer.getbuffer(),
        )
...
def test_create_partitions_response_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_create_partitions_response(
            buffer.getbuffer(),
        )
