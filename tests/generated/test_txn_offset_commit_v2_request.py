...
from kio.schema.txn_offset_commit.v2.request import TxnOffsetCommitRequest
from kio.schema.txn_offset_commit.v2.request import TxnOffsetCommitRequestPartition
from kio.schema.txn_offset_commit.v2.request import TxnOffsetCommitRequestTopic
...
def test_txn_offset_commit_request_partition_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_txn_offset_commit_request_partition(
            buffer.getbuffer(),
        )
...
def test_txn_offset_commit_request_topic_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_txn_offset_commit_request_topic(
            buffer.getbuffer(),
        )
...
def test_txn_offset_commit_request_roundtrip(instance: TxnOffsetCommitRequest) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_txn_offset_commit_request(
            buffer.getbuffer(),
        )
