...
from kio.schema.stop_replica.v0.response import StopReplicaPartitionError
from kio.schema.stop_replica.v0.response import StopReplicaResponse
...
def test_stop_replica_partition_error_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_stop_replica_partition_error(
            buffer.getbuffer(),
        )
...
def test_stop_replica_response_roundtrip(instance: StopReplicaResponse) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_stop_replica_response(
            buffer.getbuffer(),
        )
