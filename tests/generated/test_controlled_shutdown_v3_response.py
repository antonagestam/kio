...
from kio.schema.controlled_shutdown.v3.response import ControlledShutdownResponse
from kio.schema.controlled_shutdown.v3.response import RemainingPartition
...
def test_remaining_partition_roundtrip(instance: RemainingPartition) -> None:
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_remaining_partition(
            buffer.getbuffer(),
        )
...
def test_controlled_shutdown_response_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_controlled_shutdown_response(
            buffer.getbuffer(),
        )
