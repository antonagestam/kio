...
from kio.schema.leader_and_isr.v6.request import LeaderAndIsrLiveLeader
from kio.schema.leader_and_isr.v6.request import LeaderAndIsrPartitionState
from kio.schema.leader_and_isr.v6.request import LeaderAndIsrRequest
from kio.schema.leader_and_isr.v6.request import LeaderAndIsrTopicState
...
def test_leader_and_isr_partition_state_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_leader_and_isr_partition_state(
            buffer.getbuffer(),
        )
...
def test_leader_and_isr_topic_state_roundtrip(instance: LeaderAndIsrTopicState) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_leader_and_isr_topic_state(
            buffer.getbuffer(),
        )
...
def test_leader_and_isr_live_leader_roundtrip(instance: LeaderAndIsrLiveLeader) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_leader_and_isr_live_leader(
            buffer.getbuffer(),
        )
...
def test_leader_and_isr_request_roundtrip(instance: LeaderAndIsrRequest) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_leader_and_isr_request(
            buffer.getbuffer(),
        )
