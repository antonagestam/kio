...
from kio.schema.elect_leaders.v2.response import PartitionResult
from kio.schema.elect_leaders.v2.response import ReplicaElectionResult
...
def test_partition_result_roundtrip(instance: PartitionResult) -> None:
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_partition_result(
            buffer.getbuffer(),
        )
...
def test_replica_election_result_roundtrip(instance: ReplicaElectionResult) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_replica_election_result(
            buffer.getbuffer(),
        )
...
def test_elect_leaders_response_roundtrip(instance: ElectLeadersResponse) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_elect_leaders_response(
            buffer.getbuffer(),
        )
