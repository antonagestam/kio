...
from kio.schema.list_offsets.v5.request import ListOffsetsPartition
from kio.schema.list_offsets.v5.request import ListOffsetsRequest
from kio.schema.list_offsets.v5.request import ListOffsetsTopic
...
def test_list_offsets_partition_roundtrip(instance: ListOffsetsPartition) -> None:
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_list_offsets_partition(
            buffer.getbuffer(),
        )
...
def test_list_offsets_topic_roundtrip(instance: ListOffsetsTopic) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_list_offsets_topic(
            buffer.getbuffer(),
        )
...
def test_list_offsets_request_roundtrip(instance: ListOffsetsRequest) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_list_offsets_request(
            buffer.getbuffer(),
        )
