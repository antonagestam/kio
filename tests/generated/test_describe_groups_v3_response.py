...
from kio.schema.describe_groups.v3.response import DescribedGroup
from kio.schema.describe_groups.v3.response import DescribedGroupMember
from kio.schema.describe_groups.v3.response import DescribeGroupsResponse
...
def test_described_group_member_roundtrip(instance: DescribedGroupMember) -> None:
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_described_group_member(
            buffer.getbuffer(),
        )
...
def test_describe_groups_response_roundtrip(instance: DescribeGroupsResponse) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_describe_groups_response(
            buffer.getbuffer(),
        )
