...
from kio.schema.create_partitions.v0.request import CreatePartitionsAssignment
from kio.schema.create_partitions.v0.request import CreatePartitionsRequest
from kio.schema.create_partitions.v0.request import CreatePartitionsTopic
...
def test_create_partitions_assignment_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_create_partitions_assignment(
            buffer.getbuffer(),
        )
...
def test_create_partitions_topic_roundtrip(instance: CreatePartitionsTopic) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_create_partitions_topic(
            buffer.getbuffer(),
        )
...
def test_create_partitions_request_roundtrip(instance: CreatePartitionsRequest) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_create_partitions_request(
            buffer.getbuffer(),
        )
