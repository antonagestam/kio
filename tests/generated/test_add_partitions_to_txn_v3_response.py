...
from kio.schema.add_partitions_to_txn.v3.response import AddPartitionsToTxnResponse
from kio.schema.add_partitions_to_txn.v3.response import AddPartitionsToTxnTopicResult
...
def test_add_partitions_to_txn_partition_result_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_add_partitions_to_txn_partition_result(
            buffer.getbuffer(),
        )
...
def test_add_partitions_to_txn_topic_result_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_add_partitions_to_txn_topic_result(
            buffer.getbuffer(),
        )
...
def test_add_partitions_to_txn_response_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_add_partitions_to_txn_response(
            buffer.getbuffer(),
        )
