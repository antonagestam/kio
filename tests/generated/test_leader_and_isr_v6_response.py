...
from kio.schema.leader_and_isr.v6.response import LeaderAndIsrPartitionError
from kio.schema.leader_and_isr.v6.response import LeaderAndIsrResponse
from kio.schema.leader_and_isr.v6.response import LeaderAndIsrTopicError
...
def test_leader_and_isr_partition_error_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_leader_and_isr_partition_error(
            buffer.getbuffer(),
        )
...
def test_leader_and_isr_topic_error_roundtrip(instance: LeaderAndIsrTopicError) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_leader_and_isr_topic_error(
            buffer.getbuffer(),
        )
...
def test_leader_and_isr_response_roundtrip(instance: LeaderAndIsrResponse) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_leader_and_isr_response(
            buffer.getbuffer(),
        )
