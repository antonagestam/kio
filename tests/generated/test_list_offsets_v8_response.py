...
from kio.schema.list_offsets.v8.response import ListOffsetsPartitionResponse
from kio.schema.list_offsets.v8.response import ListOffsetsResponse
from kio.schema.list_offsets.v8.response import ListOffsetsTopicResponse
...
def test_list_offsets_partition_response_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_list_offsets_partition_response(
            buffer.getbuffer(),
        )
...
def test_list_offsets_topic_response_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_list_offsets_topic_response(
            buffer.getbuffer(),
        )
...
def test_list_offsets_response_roundtrip(instance: ListOffsetsResponse) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_list_offsets_response(
            buffer.getbuffer(),
        )
