...
def test_described_delegation_token_renewer_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_described_delegation_token_renewer(
            buffer.getbuffer(),
        )
...
def test_described_delegation_token_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_described_delegation_token(
            buffer.getbuffer(),
        )
...
def test_describe_delegation_token_response_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_describe_delegation_token_response(
            buffer.getbuffer(),
        )
