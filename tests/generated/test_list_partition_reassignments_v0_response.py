...
def test_ongoing_partition_reassignment_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_ongoing_partition_reassignment(
            buffer.getbuffer(),
        )
...
def test_ongoing_topic_reassignment_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_ongoing_topic_reassignment(
            buffer.getbuffer(),
        )
...
def test_list_partition_reassignments_response_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_list_partition_reassignments_response(
            buffer.getbuffer(),
        )
