...
from kio.schema.offset_fetch.v7.response import OffsetFetchResponse
from kio.schema.offset_fetch.v7.response import OffsetFetchResponsePartition
from kio.schema.offset_fetch.v7.response import OffsetFetchResponseTopic
...
def test_offset_fetch_response_partition_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_offset_fetch_response_partition(
            buffer.getbuffer(),
        )
...
def test_offset_fetch_response_topic_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_offset_fetch_response_topic(
            buffer.getbuffer(),
        )
