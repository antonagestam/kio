...
from kio.schema.update_metadata.v7.request import UpdateMetadataBroker
from kio.schema.update_metadata.v7.request import UpdateMetadataEndpoint
from kio.schema.update_metadata.v7.request import UpdateMetadataPartitionState
from kio.schema.update_metadata.v7.request import UpdateMetadataRequest
from kio.schema.update_metadata.v7.request import UpdateMetadataTopicState
...
def test_update_metadata_topic_state_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_update_metadata_topic_state(
            buffer.getbuffer(),
        )
...
def test_update_metadata_endpoint_roundtrip(instance: UpdateMetadataEndpoint) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_update_metadata_endpoint(
            buffer.getbuffer(),
        )
