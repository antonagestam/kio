...
from kio.schema.produce.v10.request import PartitionProduceData
from kio.schema.produce.v10.request import ProduceRequest
from kio.schema.produce.v10.request import TopicProduceData
...
def test_partition_produce_data_roundtrip(instance: PartitionProduceData) -> None:
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_partition_produce_data(
            buffer.getbuffer(),
        )
...
def test_topic_produce_data_roundtrip(instance: TopicProduceData) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_topic_produce_data(
            buffer.getbuffer(),
        )
...
def test_produce_request_roundtrip(instance: ProduceRequest) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_produce_request(
            buffer.getbuffer(),
        )
