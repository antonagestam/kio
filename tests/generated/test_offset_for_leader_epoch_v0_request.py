...
from kio.schema.offset_for_leader_epoch.v0.request import OffsetForLeaderEpochRequest
from kio.schema.offset_for_leader_epoch.v0.request import OffsetForLeaderPartition
from kio.schema.offset_for_leader_epoch.v0.request import OffsetForLeaderTopic
...
def test_offset_for_leader_partition_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_offset_for_leader_partition(
            buffer.getbuffer(),
        )
...
def test_offset_for_leader_topic_roundtrip(instance: OffsetForLeaderTopic) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_offset_for_leader_topic(
            buffer.getbuffer(),
        )
...
def test_offset_for_leader_epoch_request_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_offset_for_leader_epoch_request(
            buffer.getbuffer(),
        )
