...
from kio.schema.txn_offset_commit.v2.response import TxnOffsetCommitResponse
from kio.schema.txn_offset_commit.v2.response import TxnOffsetCommitResponsePartition
from kio.schema.txn_offset_commit.v2.response import TxnOffsetCommitResponseTopic
...
def test_txn_offset_commit_response_partition_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_txn_offset_commit_response_partition(
            buffer.getbuffer(),
        )
...
def test_txn_offset_commit_response_topic_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_txn_offset_commit_response_topic(
            buffer.getbuffer(),
        )
...
def test_txn_offset_commit_response_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_txn_offset_commit_response(
            buffer.getbuffer(),
        )
