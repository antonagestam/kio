...
def test_credential_info_roundtrip(instance: CredentialInfo) -> None:
    ...
    with setup_buffer() as buffer:
        ...
        remaining, result = read_credential_info(
            buffer.getbuffer(),
        )
...
def test_describe_user_scram_credentials_result_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_describe_user_scram_credentials_result(
            buffer.getbuffer(),
        )
...
def test_describe_user_scram_credentials_response_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        remaining, result = read_describe_user_scram_credentials_response(
            buffer.getbuffer(),
        )
